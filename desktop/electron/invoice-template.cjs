...
function generateInvoiceHTML({ invoice, items, property, client, agency, guide }) {
  const p = property || {};
  const inv = invoice || {};
  const rows = items || [];
  ...
  const logoTag = p.logo_path
    ? `<img src="file://${p.logo_path}" style="width:65px;height:55px;object-fit:contain;flex-shrink:0">`
    : `<div style="width:65px;height:55px;flex-shrink:0"></div>`;

  const logoFooter = p.logo_path
    ? `<img src="file://${p.logo_path}" style="height:30px;object-fit:contain;display:block;margin:0 auto 6px">`
    : '';
