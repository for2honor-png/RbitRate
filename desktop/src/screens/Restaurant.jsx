...
function KitchenTab({ propertyId }) {
  ...
  const load = useCallback(async () => {
    const [ord, shifts] = await Promise.all([
      window.db.invoke('orders:getActive', { property_id: propertyId }),
      window.db.invoke('shifts:getActive', { property_id: propertyId }),
    ]);
    setOrders(ord || []);
  ...
  useEffect(() => {
    const unsub = window.db.onOrderNew(() => {
      setNewAlert(true);
  ...
  async function handleAdvance(orderId, newStatus) {
    await window.db.invoke('orders:updateStatus', {
...
function FormulaModal({ formulaId, propertyId, onSave, onClose }) {
  ...
  useEffect(() => {
    if (!isEdit) return;
    window.db.invoke('formulas:getWithCourses', { id: formulaId }).then(f => {
      if (!f) return;
  ...
  async function handleSave() {
    ...
    setSaving(true);
    if (isEdit) {
      await window.db.invoke('formulas:update', { id: formulaId, ...form, price: parseFloat(form.price) });
    } else {
      const { id } = await window.db.invoke('formulas:create', { ...form, price: parseFloat(form.price), property_id: propertyId });
      // Save courses + options
      for (let ci = 0; ci < courses.length; ci++) {
        const c = courses[ci];
        const { id: cid } = await window.db.invoke('formulas:addCourse', {
        ...
        for (let oi = 0; oi < (c.options || []).length; oi++) {
          ...
          await window.db.invoke('formulas:addOption', { course_id: cid, name_fr: o.name_fr, sort_order: oi });
        }
      }
    }
    setSaving(false);
  ...
  async function handleAddCourse() {
    ...
    if (isEdit) {
      const { id } = await window.db.invoke('formulas:addCourse', {
  ...
  async function handleDeleteCourse(idx) {
    const c = courses[idx];
    if (c.id && isEdit) await window.db.invoke('formulas:deleteCourse', { id: c.id });
  ...
  async function handleAddOption(cIdx) {
    ...
    if (isEdit && courses[cIdx]?.id) {
      const { id } = await window.db.invoke('formulas:addOption', {
  ...
  async function handleDeleteOption(cIdx, oIdx) {
    ...
    if (opt?.id && isEdit) await window.db.invoke('formulas:deleteOption', { id: opt.id });
...
function FormulasSection({ propertyId }) {
  ...
  const load = useCallback(async () => {
    const data = await window.db.invoke('formulas:getAll', { property_id: propertyId });
    setFormulas(data || []);
  }, [propertyId]);
  ...
  async function handleToggle(f) {
    await window.db.invoke('formulas:toggle', { id: f.id, active: !f.active });
    load();
  }

  async function handleDelete(id) {
    await window.db.invoke('formulas:delete', { id });
    load();
  }
    ...
    <div style={{ marginBottom: 28 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 14 }}>
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: 12, color: theme.dark, opacity: 0.45 }}>Prix fixe tout compris, visible en premier sur le QR</div>
        </div>
...
function MenuTab({ propertyId }) {
  ...
  const load = useCallback(async () => {
    const cats = await window.db.invoke('menu:getCategories', { property_id: propertyId });
    setCategories(cats || []);
    const map = {};
    for (const cat of (cats || [])) {
      map[cat.id] = await window.db.invoke('menu:getItems', { category_id: cat.id });
    }
  ...
  async function handleCreateCategory(form) {
    await window.db.invoke('menu:createCategory', { ...form, property_id: propertyId });
  ...
  async function handleSaveItem(form) {
    if (editingItem) {
      await window.db.invoke('menu:updateItem', { id: editingItem.id, ...form });
    } else {
      await window.db.invoke('menu:createItem', { ...form, category_id: addingItemCatId });
    }
    setEditingItem(null);
  ...
  async function handleToggle(item) {
    await window.db.invoke('menu:toggleAvailable', { id: item.id, available: !item.available });
    load();
  }

  async function handleDeleteItem(id) {
    await window.db.invoke('menu:deleteItem', { id });
    load();
  }

  async function handleDeleteCategory(id) {
    await window.db.invoke('menu:deleteCategory', { id });
    load();
  }

  return (
    <div>
      {/* Formulas section always at top */}
...
function QRTab({ propertyId }) {
  ...
  const load = useCallback(async () => {
    const [tbls, info] = await Promise.all([
      window.db.invoke('tables:getAll', { property_id: propertyId }),
      window.db.invoke('server:getLocalIP', {}),
    ]);
    setTables(tbls || []);
    setServerInfo(info);
    ...
    const imgs = {};
    for (const t of (tbls || [])) {
      const url = `http://${info.ip}:${info.port}/menu?p=${propertyId}&t=${t.id}&l=${encodeURIComponent(t.label)}`;
      const { dataUrl } = await window.db.invoke('qr:generate', { url });
      imgs[t.id] = { dataUrl, url };
    }
    setQrImages(imgs);
  }, [propertyId]);
  ...
  async function handleAdd() {
    ...
    setAdding(true);
    await window.db.invoke('tables:create', { property_id: propertyId, label: newLabel.trim() });
    setNewLabel('');
    setAdding(false);
    load();
  }
  ...
  async function handleDelete(id) {
    await window.db.invoke('tables:delete', { id });
    load();
  }

  function downloadQR(tableId, label) {
    const img = qrImages[tableId];
    if (!img) return;
  ...
  function copyUrl(tableId) {
    const img = qrImages[tableId];
    if (img) navigator.clipboard.writeText(img.url).catch(() => {});
  }
  ...
  return (
    <div>
      <div style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: 10, padding: '12px 16px', marginBottom: 20, display: 'flex', alignItems: 'center', gap: 12 }}>
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 12, fontWeight: 700, color: '#166534', textTransform: 'uppercase', letterSpacing: 0.4, marginBottom: 2 }}>Adresse du serveur de commandes</div>
          <div style={{ fontFamily: 'monospace', fontSize: 15, color: '#15803d', fontWeight: 700 }}>{serverUrl}</div>
          <div style={{ fontSize: 11, color: '#166534', opacity: 0.7, marginTop: 2 }}>Les clients doivent être sur le même réseau WiFi</div>
        </div>
        <button
          onClick={() => navigator.clipboard.writeText(serverUrl).catch(() => {})}
          style={{ ...BTN_GHOST, fontSize: 12, padding: '6px 12px', flexShrink: 0 }}
        >
          📋 Copier
        </button>
      </div>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 24 }}>
        <input
          ...
          placeholder="ex: Table 1, Terrasse A, Chambre 101..."
          style={{ ...FIELD, flex: 1 }}
        />
        <button onClick={handleAdd} disabled={adding || !newLabel.trim()} style={{ ...BTN_PRIMARY, flexShrink: 0, opacity: adding ? 0.7 : 1 }}>
          + Ajouter
        </button>
      </div>
      ...
      {tables.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '60px 20px', color: theme.dark, opacity: 0.35 }}>
          <div style={{ fontSize: 32, marginBottom: 10 }}>🪑</div>
          <div style={{ fontSize: 15, fontWeight: 600 }}>Aucune table</div>
          <div style={{ fontSize: 13, marginTop: 4 }}>Ajoutez des tables pour générer les QR codes</div>
        </div>
      ) : (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(170px, 1fr))', gap: 14 }}>
          {tables.map(table => {
            const qr = qrImages[table.id];
            return (
              <div key={table.id} style={{
                background: theme.white, borderRadius: 12, padding: 14,
                border: '1px solid rgba(31,42,46,0.08)', textAlign: 'center',
              }}>
                {qr ? (
                  <img src={qr.dataUrl} alt={`QR ${table.label}`} style={{ width: '100%', maxWidth: 140, height: 'auto', borderRadius: 6 }} />
                ) : (
                  <div style={{ width: '100%', aspectRatio: '1', background: theme.cream, borderRadius: 6, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22 }}>⏳</div>
                )}
                <div style={{ fontWeight: 700, fontSize: 13, color: theme.dark, marginTop: 8 }}>{table.label}</div>
                {qr && (
                  <div style={{ fontSize: 10, color: theme.dark, opacity: 0.45, wordBreak: 'break-all', marginTop: 3, marginBottom: 6, fontFamily: 'monospace' }}>
                    {qr.url}
                  </div>
                )}
                <div style={{ display: 'flex', gap: 6, justifyContent: 'center', marginTop: qr ? 0 : 10 }}>
                  <button onClick={() => downloadQR(table.id, table.label)} title="Télécharger" style={{ ...BTN_GHOST, padding: '5px 10px', fontSize: 13, flex: 1 }}>⬇</button>
                  <button onClick={() => copyUrl(table.id)} title="Copier le lien" style={{ ...BTN_GHOST, padding: '5px 10px', fontSize: 13, flex: 1 }}>🔗</button>
                  <button onClick={() => handleDelete(table.id)} title="Supprimer" style={{ ...BTN_DANGER, padding: '5px 8px', fontSize: 13 }}>✕</button>
                </div>
              </div>
            );
          })}
...
export default function Restaurant() {
  ...
  useEffect(() => {
    ...
    window.db.invoke('properties:getAll', {}).then(props => {
  ...
  if (!selectedPropertyId) {
    return (
      <div style={{ textAlign: 'center', marginTop: 80, color: theme.dark, opacity: 0.35 }}>
        <div style={{ fontSize: 40, marginBottom: 12 }}>🍽</div>
  ...
  if (!property.restaurant_active) {
    return (
      <div style={{ textAlign: 'center', marginTop: 80, color: theme.dark }}>
  ...
  return (
    <div style={{ fontFamily: theme.font }}>
      ...
      {tab === 2 && <QRTab propertyId={selectedPropertyId} />}
    </div>
  );
}
