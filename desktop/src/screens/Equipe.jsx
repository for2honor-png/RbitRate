...
function StaffModal({ initial, onClose, onSaved }) {
  ...
  async function save() {
    ...
    try {
      ...
      if (isEdit) {
        ...
        await window.db.invoke('staff:update', args);
      } else {
        await window.db.invoke('staff:create', {
  ...
  return (
    <Modal title={isEdit ? 'Modifier le membre' : 'Ajouter un membre'} onClose={onClose} width={520}>
...
export default function Equipe() {
  ...
  async function load() {
    setLoading(true);
    const list = await window.db.invoke('staff:getFull', {});
    setStaffList(list);
    setLoading(false);
  }
  ...
  async function deactivate(member) {
    await window.db.invoke('staff:deactivate', { id: member.id });
    setConfirm(null);
    load();
  }
  ...
  return (
    <div style={{ maxWidth: 780, margin: '0 auto' }}>
      ...
      {confirm && (
        <Modal title="Désactiver ce membre" onClose={() => setConfirm(null)} width={380}>
