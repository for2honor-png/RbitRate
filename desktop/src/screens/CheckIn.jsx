...
export default function CheckIn() {
  ...
  const [roomAssignments, setRoomAssignments] = useState({}); // { room_id: [person_idx...] }
  ...
  const [savedGuestResMap, setSavedGuestResMap] = useState({}); // { guestId: reservationId }
  ...
  useEffect(() => {
    ...
    window.db.invoke('rooms:getWithCapacity', { property_id: selectedPropertyId })
      .then(r => setRoomsWithCapacity(r || []))
      .catch(() =>
        window.db.invoke('rooms:getAll', { property_id: selectedPropertyId })
  ...
  useEffect(() => {
    if (checkInRoomId) {
      ...
      setGroupShared(g => ({ ...g, room_id: checkInRoomId }));
    }
  }, [checkInRoomId]);
  ...
  useEffect(() => {
    ...
    searchTimer.current = setTimeout(() => {
      window.db.invoke('guests:search', { query: query.trim() }).then(setResults);
    }, 300);
  ...
  // Rooms that have at least one person currently assigned
  ...
  async function handleConfirm() {
    ...
    try {
      if (isPartialRoom) {
        ...
        if (!result.ok) { showToast('Erreur : ' + result.error); setSaving(false); return; }
        ...
        if (form.room_id && form.check_in_date && form.check_out_date && selectedPropertyId) {
          const avail = await window.db.invoke('channels:checkAvailability', {
        ...
        if (isNewGuest) {
          const { id } = await window.db.invoke('guests:create', {
            last_name: form.last_name, first_name: form.first_name,
        ...
        const { id: reservationId } = await window.db.invoke('reservations:create', {
  ...
  async function handlePrintFiche() {
    ...
    try {
      const result = await window.db.invoke('fiche:print', { reservationId: savedReservationId });
  ...
  async function handleSaveFichePDF() {
    if (!savedReservationId) return;
    setSavingPDF(true); setPrintError(null);
    try {
      const result = await window.db.invoke('fiche:savePDF', { reservationId: savedReservationId });
      if (result?.ok) {
        showToast('✓ PDF enregistré: ' + (result.path?.split(/[/\\]/).pop() || 'fiche.pdf'));
      } else if (result?.error && result.error !== 'Annulé') {
        setPrintError(result.error);
      }
    } catch { setPrintError('Erreur PDF'); }
    setSavingPDF(false);
  }
  ...
  async function handleGroupConfirm() {
    ...
    try {
      ...
      const { id: grpId } = await window.db.invoke('groups:create', {
      ...
      const byRoom = {}; // { room_id: [{ guestId, isChef }] }
      ...
      for (const [roomId, members] of Object.entries(byRoom)) {
        ...
        if (room.status === 'available') {
          const { id: resId } = await window.db.invoke('reservations:create', {
          ...
          for (const m of members) {
            if (m.guestId !== chef.guestId) {
              await window.db.invoke('reservation_guests:add', {
          ...
          // Partial room: add all to existing reservation
  ...
  async function handlePrintIndividuals() {
    ...
    for (const gId of savedGroupGuestIds) {
      const resId = savedGuestResMap[gId] || savedGroupResId;
      if (resId) {
        await window.db.invoke('fiche:printForGuest', { reservationId: resId, guestId: gId });
      }
    }
  ...
  async function handlePrintGroupTable() {
    if (!savedGroupId && !savedGroupResId) return;
    setPrintingGroupTable(true);
    try {
      const result = await window.db.invoke('fiche:printGroup', {
        groupId: savedGroupId || undefined,
        reservationId: savedGroupId ? undefined : savedGroupResId,
      });
      if (result && !result.ok) showToast('Erreur impression : ' + result.error);
    } catch { showToast('Erreur impression tableau'); }
    ...
    return (
      <div style={{ fontFamily: theme.font, maxWidth: 800 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', marginBottom: 24, gap: 12 }}>
          <button onClick={() => { setCheckInType('individual'); setSavedGroupId(null); setSavedGroupResId(null); setSavedGroupGuestIds([]); setSavedGuestResMap({}); setGroupPersons([{ ...emptyGroupPerson(), isChef: true }]); setRoomAssignments({}); setGroupInfo({ name: '', visit_name: '', agency_id: null, guide_id: null, leader_name: '', leader_phone: '', leader_role: 'guide' }); setAgencyQuery(''); setAgencySelected(null); setAgencySuggestions([]); setGuideOptions([]); setShowNewGuideForm(false); setNewGuide({ name: '', phone: '', role: 'guide' }); }}
                    ...
                    <div style={{
                      ...
                      <div style={{ display: 'flex', gap: 8 }}>
                        ...
                        <button onClick={async () => {
                          ...
                          const result = await window.db.invoke('guides:create', {
                            agency_id: groupInfo.agency_id || null,
  ...
  return (
    <div style={{ fontFamily: theme.font, maxWidth: 760 }}>
      ...
      <div style={{ background: theme.white, borderRadius: 16, padding: 28, border: '1px solid rgba(31,42,46,0.08)', boxShadow: '0 4px 24px rgba(31,42,46,0.06)' }}>
        ...
        {/* Section 5: Paiement (hidden for partial room add) */}
        {!isPartialRoom && (
          <>
