...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    window.db.invoke('rooms:getAll', { property_id: selectedPropertyId }).then(setRooms);
    window.db.invoke('reservations:getTodayStats', { property_id: selectedPropertyId }).then(setTodayStats);
    window.db.invoke('finance:getTodayTotals', { property_id: selectedPropertyId }).then(setTodayFinance);
    window.db.invoke('shifts:getActive', { property_id: selectedPropertyId }).then(setActiveShifts);
    window.db.invoke('finance:getRecentTransactions', { property_id: selectedPropertyId, limit: 4 }).then(setRecentTxns);
  }, [selectedPropertyId]);

  useEffect(() => {
    if (!staff?.id) return;
    const mon = toISO(getMonday(new Date()));
    window.db.invoke('schedules:getTodayForStaff', { staff_id: staff.id }).then(setTodaySchedule);
    window.db.invoke('schedules:getWeekForStaff',  { staff_id: staff.id, start_date: mon }).then(setWeekSchedule);
  }, [staff?.id]);

  // Listen for shift reminder events from main process
  useEffect(() => {
    const unsub = window.db.on('schedule:shiftReminder', (data) => {
  ...
  // Build the 7-day mini calendar for the current user
