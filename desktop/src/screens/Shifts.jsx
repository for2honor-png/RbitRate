...
function CloseShiftModal({ shift, onConfirm, onClose }) {
  ...
  async function handleClose() {
    setSaving(true);
    const res = await window.db.invoke('shifts:close', {
      shift_id: shift.id,
      closing_cash: parseFloat(closingCash) || 0,
    });
    setSaving(false);
    if (res.ok) { setResult(res); setStep(2); }
  }
...
function AddTxnModal({ shift, defaultType, onConfirm, onClose }) {
  ...
  async function handleSave() {
    ...
    setSaving(true);
    const result = await window.db.invoke('transactions:create', {
      shift_id: shift.id,
...
export default function Shifts() {
  ...
  const load = useCallback(async () => {
    ...
    const [shifts, recent] = await Promise.all([
      window.db.invoke('shifts:getActive', { property_id: selectedPropertyId }),
      window.db.invoke('finance:getRecentTransactions', { property_id: selectedPropertyId, limit: 200 }),
    ]);
  ...
  async function handleOpen() {
    ...
    const result = await window.db.invoke('shifts:open', {
      staff_id: staff.id,
      property_id: selectedPropertyId,
      opening_cash: parseFloat(openingCash) || 0,
    });
    setOpening(false);
    if (result.ok) { setOpeningCash(''); showToast('✓ Shift ouvert'); await load(); }
    else setOpenError(result.error || 'Erreur');
  }
