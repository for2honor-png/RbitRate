...
function AgenciesView({ showToast }) {
  ...
  const loadAgencies = useCallback(() => {
    invoke('agencies:getAll').then(r => { if (r) setAgencies(r); });
  }, []);
  ...
  async function toggleExpand(agency) {
    ...
    if (!expanded[id]) {
      const [gs, vs] = await Promise.all([
        invoke('guides:getAll', { agency_id: id }),
        invoke('agencies:getGroups', { agency_id: id }),
      ]);
      setGuides(g => ({ ...g, [id]: gs || [] }));
  ...
  async function handleSaveAgency(form) {
    if (modal?.data) {
      await invoke('agencies:update', { id: modal.data.id, ...form });
      showToast('Agence modifiée');
    } else {
      await invoke('agencies:create', form);
  ...
  async function handleDeleteAgency(id) {
    await invoke('agencies:delete', { id });
  ...
  async function handleSaveGuide(form) {
    if (modal?.guideData) {
      await invoke('guides:update', { id: modal.guideData.id, ...form });
      showToast('Guide modifié');
    } else {
      await invoke('guides:create', form);
      showToast('Guide ajouté');
    }
    setModal(null);
    const gs = await invoke('guides:getAll', { agency_id: form.agency_id });
    setGuides(g => ({ ...g, [form.agency_id]: gs || [] }));
  }

  async function handleDeleteGuide(guide) {
    await invoke('guides:delete', { id: guide.id });
    showToast('Guide supprimé');
    const gs = await invoke('guides:getAll', { agency_id: guide.agency_id });
...
export default function Guests() {
  ...
  const load = useCallback(() => {
    ...
    if (query.trim().length >= 1) {
      invoke('guests:search', { query: query.trim() }).then(r => { if (r) setGuests(r); });
    } else {
      invoke('guests:getAll', { tag: tagFilter }).then(r => { if (r) setGuests(r); });
    }
  ...
  async function handleSave(form) {
    if (editing) {
      await invoke('guests:update', { id: editing.id, ...form });
      showToast('Client modifié avec succès');
    } else {
      await invoke('guests:create', form);
