...
function OtaModal({ ota, propertyId, onSave, onClose }) {
  ...
  async function handleTest() {
    ...
    setTesting(true);
    setTestResult(null);
    const result = await invoke('channels:testURL', { url: importUrl.trim() });
    setTesting(false);
        ...
        <div>
          ...
          {testResult && (
            <div style={{
              ...
              background: testResult && testResult.ok ? '#f0fdf4' : '#fef2f2',
              color: testResult && testResult.ok ? '#16a34a' : theme.coral,
              border: `1px solid ${testResult && testResult.ok ? '#86efac' : '#fca5a5'}`,
            }}>
              {testResult && testResult.ok
                ? `✓ ${testResult.eventCount} événements trouvés — URL valide`
                : `✗ ${testResult?.error || 'Erreur de test'}`}
            </div>
          )}
        </div>
...
function CalendarView({ propertyId, otas }) {
  ...
  const load = useCallback(async () => {
    ...
    const data = await invoke('channels:getCalendar', { property_id: propertyId, year, month });
  ...
  useEffect(() => {
    const unsub = on('channels:syncComplete', ({ property_id }) => {
  ...
  async function handleSyncAll() {
    if (!propertyId) return;
    setSyncing(true);
    await invoke('channels:syncAll', { property_id: propertyId });
    await load();
    setSyncing(false);
  }
...
function ConnectionsTab({ propertyId, otas, onRefresh }) {
  ...
  async function handleSyncOne(ota) {
    ...
    const result = await invoke('channels:syncOne', { property_id: propertyId, ota_id: ota.id });
    setSyncing(s => ({ ...s, [ota.id]: false }));
    if (result && result.ok) showToast(`✓ ${ota.name}: +${result.added} nouvelles réservations`);
    else showToast(`Erreur ${ota.name}: ${result?.error || 'Erreur'}`, true);
    onRefresh();
  }

  async function handleRemove(ota) {
    if (!confirm(`Supprimer la connexion ${ota.name} ?`)) return;
    await invoke('channels:removeOTA', { property_id: propertyId, ota_id: ota.id });
    onRefresh();
  }

  async function handleExport() {
    showToast('Export .ics non disponible en version web', true);
  }
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button onClick={handleExport} style={{
            ...btnBase, fontWeight: 600, fontSize: 12, opacity: 0.5, cursor: 'not-allowed',
          }} title="Export .ics non disponible en version web">📥 Télécharger export .ics</button>
...
export default function Channels() {
  ...
  async function loadOtas() {
    ...
    const props = await invoke('properties:getAll', {});
