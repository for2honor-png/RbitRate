...
function LocalModal({ title, onClose, children, width = 520 }) {
  return (
...
function CalendarTab({ propertyId }) {
  ...
  const load = useCallback(async () => {
    ...
    const [s, st, tmpl] = await Promise.all([
      invoke('schedules:getRange', { property_id: propertyId, start_date: startStr, end_date: endStr }),
      invoke('staff:getAll'),
      invoke('templates:getAll', { property_id: propertyId }),
    ]);
    setSchedules(s || []);
    setAllStaff(st || []);
    setTemplates(tmpl || []);
...
function GenerateModal({ propertyId, templates, defaultStart, staffId, onGenerated, onClose }) {
  ...
  async function generate() {
    ...
    setLoading(true);
    const res = await invoke('schedules:autoGenerate', {
      property_id: propertyId, start_date: startDate,
      weeks, template_id: templateId, created_by: staffId,
    });
    setLoading(false);
    setResult(res);
    if (res && res.ok) setTimeout(onGenerated, 1500);
  }

  return (
    <LocalModal title="⚡ Générer le planning" onClose={onClose}>
      {templates.length === 0 ? (
...
function TemplateModal({ initial, propertyId, onClose, onSaved }) {
  ...
  async function save() {
    ...
    setSaving(true);
    if (isEdit) {
      await invoke('templates:update', { id: initial.id, name, pattern, cycle_days: pattern.length, day_shift_start: dayStart, day_shift_end: dayEnd, night_shift_start: nightStart, night_shift_end: nightEnd });
    } else {
      await invoke('templates:create', { property_id: propertyId, name, pattern, cycle_days: pattern.length, day_shift_start: dayStart, day_shift_end: dayEnd, night_shift_start: nightStart, night_shift_end: nightEnd });
    }
    setSaving(false);
    onSaved();
    onClose();
  ...
  return (
    <LocalModal title={isEdit ? 'Modifier le modèle' : 'Nouveau modèle'} onClose={onClose} width={560}>
      <div style={{ marginBottom: 14 }}>
...
function TemplatesTab({ propertyId }) {
  ...
  const load = useCallback(async () => {
    if (!propertyId) return;
    const data = await invoke('templates:getAll', { property_id: propertyId });
    setTemplates(data || []);
  }, [propertyId]);
...
function TimeoffTab({ propertyId }) {
  ...
  const load = useCallback(async () => {
    ...
    const [r, s] = await Promise.all([
      invoke('timeoff:getAll', { property_id: propertyId }),
      invoke('staff:getAll'),
    ]);
    setRequests(r || []);
    setAllStaff(s || []);
  ...
  async function review(id, status) {
    await invoke('timeoff:review', { id, status, reviewed_by: currentStaff?.id });
    load();
  }

  async function del(id) {
    if (!confirm('Supprimer cette demande ?')) return;
    await invoke('timeoff:delete', { id });
    load();
  }
...
function AddTimeoffModal({ propertyId, allStaff, onClose, onSaved }) {
  ...
  return (
    <LocalModal title="Ajouter un congé / absence" onClose={onClose}>
      <div style={{ marginBottom: 14 }}>
