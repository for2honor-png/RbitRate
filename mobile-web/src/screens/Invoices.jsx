...
function QuickAddClientModal({ propertyId, onCreated, onClose }) {
  ...
  const handleSave = async () => {
    if (!name.trim()) return;
    setSaving(true);
    const result = await invoke('clients:create', {
...
function NewInvoiceModal({ property, clients, onSave, onClose }) {
  ...
  useEffect(() => {
    invoke('guests:getAll', {}).then(data => setGuests(data || []));
  }, []);
  ...
  const SEL_STYLE = { width: '100%', padding: '8px 10px', borderRadius: 7, border: '1px solid rgba(31,42,46,0.2)', fontFamily: theme.font, fontSize: 13, color: theme.dark, background: theme.white, outline: 'none', boxSizing: 'border-box' };
...
function ViewInvoiceModal({ invoiceId, onClose, onPrint }) {
  ...
  useEffect(() => {
    invoke('invoices:getById', { id: invoiceId }).then(setInv);
  }, [invoiceId]);
...
export default function Invoices() {
  ...
  const load = useCallback(async () => {
    ...
    const [inv, rec, cli, props] = await Promise.all([
      invoke('invoices:getAll', { property_id: selectedPropertyId, type: 'proforma' }),
      invoke('invoices:getAll', { property_id: selectedPropertyId, type: 'facture' }),
      invoke('clients:getAll', { property_id: selectedPropertyId }),
      invoke('properties:getAll', {}),
    ]);
    setInvoices(inv || []);
    setReceipts(rec || []);
    setClients(cli || []);
  ...
  const handleCreateInvoice = async (data) => {
    const res = await invoke('invoices:create', data);
    const created = Array.isArray(res) ? res[0] : res;
    if (created?.id) { showToast(`✓ ${created.invoice_number || 'Document'} créé`); setModal(null); await load(); }
    else showToast('Erreur lors de la création', true);
  };

  const handleAddPayment = async (data) => {
    await invoke('invoices:addPayment', data);
  ...
  const handleCreateClient = async (data) => {
    await invoke('clients:create', data);
    showToast('✓ Client créé');
    setModal(null);
    await load();
  };

  // Web: print using window.print() on a simple page
  const handlePrint = async (id) => {
    setPrinting(id);
    const inv = await invoke('invoices:getById', { id });
    setPrinting(null);
    if (inv) {
      const w = window.open('', '_blank');
      if (w) {
        w.document.write(`<html><head><title>${inv.invoice_number}</title></head><body>
          <h2>${inv.invoice_number}</h2>
          <p>Client: ${inv.client_name || inv.agency_name || 'Particulier'}</p>
          <p>Date: ${inv.date}</p>
          <p>Total TTC: ${(inv.total || 0).toFixed(2)} MAD</p>
        </body></html>`);
        w.document.close();
        w.print();
      }
