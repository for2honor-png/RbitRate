...
function PropertyModal({ initial, onSave, onClose }) {
  ...
  // Web: no logo_path support — show placeholder only
  ...
  function handleLogoChange(e) {
    ...
    // Note: logo_path not persisted to Supabase in web version
  }
  ...
  return (
    <Modal title={initial ? 'Modifier la propriété' : 'Nouvelle propriété'} onClose={onClose} width={520}>
      ...
      {tab === 0 && (
        <div>
          {/* Logo upload — web version uses file input */}
          <div style={{ marginBottom: 14 }}>
            <Label>Logo</Label>
            <label style={{
...
export default function Properties() {
  ...
  async function handleSave(form) {
    if (editing) {
      await invoke('properties:update', { ...form, id: editing.id });
    } else {
      const result = await invoke('properties:create', form);
  ...
  async function handleDelete(id) {
    await invoke('properties:delete', { id });
    load();
    refreshProperties();
