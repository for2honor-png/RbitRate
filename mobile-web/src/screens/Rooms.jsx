...
function OccupiedDetailModal({ room, reservation, selectedPropertyId, onCheckout, onStatusChange, onClose }) {
  ...
  useEffect(() => {
    if (reservation?.id) {
      invoke('reservation_guests:getAll', { reservation_id: reservation.id })
  ...
  // Print fiche — web version: open window and print
  async function handlePrintFiche() {
    if (!reservation?.id) return;
    setPrinting(true);
    setPrintError(null);
    try {
      const guestData = guests.find(g => g.is_primary) || guests[0];
      const w = window.open('', '_blank', 'width=800,height=600');
      if (w) {
        w.document.write(`<!DOCTYPE html><html><head><title>Fiche Police</title>
          <style>body{font-family:Arial,sans-serif;padding:30px;font-size:13px}h2{margin-bottom:20px}table{width:100%;border-collapse:collapse}td,th{border:1px solid #ccc;padding:6px 10px;text-align:left}th{background:#f0f0f0;font-weight:bold}</style>
          </head><body>
          <h2>Fiche Individuelle de Police</h2>
          <table>
            <tr><th>Nom</th><td>${reservation.last_name || ''} ${reservation.first_name || ''}</td></tr>
            <tr><th>Chambre</th><td>${room.room_number}</td></tr>
            <tr><th>Arrivée</th><td>${reservation.check_in_date || ''}</td></tr>
            <tr><th>Départ</th><td>${reservation.check_out_date || ''}</td></tr>
            ${guestData ? `<tr><th>Nationalité</th><td>${guestData.nationality || ''}</td></tr>
            <tr><th>N° Pièce</th><td>${guestData.document_number || ''}</td></tr>` : ''}
          </table></body></html>`);
        w.document.close();
        w.focus();
        w.print();
  ...
  // Save PDF — web version: open print dialog with PDF save option
    ...
    try {
      ...
      if (w) {
        w.document.write(`<!DOCTYPE html><html><head><title>Fiche Police</title>
          ...
          <h2>Fiche Individuelle de Police</h2>
          <table>
            <tr><th>Nom</th><td>${g.last_name || ''} ${g.first_name || ''}</td></tr>
            <tr><th>Chambre</th><td>${room.room_number}</td></tr>
            <tr><th>Réservation</th><td>${reservation.last_name || ''} ${reservation.first_name || ''}</td></tr>
            <tr><th>Arrivée</th><td>${reservation.check_in_date || ''}</td></tr>
            <tr><th>Départ</th><td>${reservation.check_out_date || ''}</td></tr>
            <tr><th>Nationalité</th><td>${g.nationality || ''}</td></tr>
            <tr><th>N° Pièce</th><td>${g.document_number || ''}</td></tr>
          </table></body></html>`);
        w.document.close();
        w.focus();
        w.print();
  ...
  async function handleRemoveGuest(rgId) {
    await invoke('reservation_guests:remove', { id: rgId });
    const rows = await invoke('reservation_guests:getAll', { reservation_id: reservation.id });
    setGuests(rows || []);
  }

  async function startCheckout() {
    if (balance > 0) {
      const shiftData = await invoke('shifts:checkOpen', { property_id: selectedPropertyId });
...
function AddGuestModal({ room, reservation, onAdded, onClose }) {
  ...
  useEffect(() => {
    ...
    setSearching(true);
    invoke('guests:search', { query }).then(r => { setResults(r || []); setSearching(false); });
  }, [query]);
  ...
  async function addExistingGuest(guest) {
    ...
    setSaving(false);
    // Supabase returns array on success, truthy check
    if (result) { onAdded(); }
...
export default function Rooms() {
  ...
  const load = useCallback(() => {
    ...
    Promise.all([
      invoke('rooms:getAll', { property_id: currentPropertyId }),
      invoke('reservations:getActive', { property_id: currentPropertyId }),
      invoke('reservation_guests:getCounts', { property_id: currentPropertyId }),
  ...
  async function handleSave(form) {
    if (editing) {
      await invoke('rooms:update', { ...form, id: editing.id });
    } else {
      await invoke('rooms:create', { ...form, property_id: currentPropertyId });
    }
    load();
    setShowModal(false);
    setEditing(null);
  ...
  async function handleDelete(id) {
    const result = await invoke('rooms:delete', { id });
    // Supabase DELETE returns empty array on success, not {ok:true}
    // Only treat non-null falsy as error if it's an explicit error object
  ...
  async function handleCheckout(reservationId, roomId) {
    await invoke('reservations:checkout', { reservation_id: reservationId, room_id: roomId });
    setDetailRoom(null);
  ...
  async function handleStatusChange(roomId, status) {
    await invoke('rooms:updateStatus', { id: roomId, status });
    setDetailRoom(null);
            ...
            <tbody>
              {rooms.length === 0 ? (
                ...
                return (
                  <tr
                    key={r.id}
                    style={{ borderBottom: i < rooms.length - 1 ? '1px solid rgba(31,42,46,0.05)' : 'none' }}
                  >
      ...
      {statusChangeRoom && (
        ...
        >
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            <button onClick={() => handleStatusChange(statusChangeRoom.id, 'available')} style={{
              padding: '12px 20px', borderRadius: 10, border: 'none',
              background: theme.teal, color: theme.white,
              fontWeight: 700, fontSize: 14, fontFamily: theme.font, cursor: 'pointer',
            }}>✅ Disponible</button>
            <button onClick={() => handleStatusChange(statusChangeRoom.id, 'cleaning')} style={{
            ...
            <button onClick={() => handleStatusChange(statusChangeRoom.id, 'maintenance')} style={{
              padding: '12px 20px', borderRadius: 10,
